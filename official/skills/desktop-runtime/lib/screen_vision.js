...
function timingMs(startedAt, endedAt = Date.now()) {
...
function runEncodedPowerShell(script, timeout = 15000) {
  const encoded = Buffer.from(script.trim(), 'utf16le').toString('base64');
  return execSync(`powershell -NoProfile -NonInteractive -EncodedCommand ${encoded}`, { timeout, encoding: 'utf8' });
}
...
/** 通过 CDP 截取浏览器当前页 */
async function takeBrowserShot(opts = {}) {
  return await cdp.screenshot({ quality: opts.fastMode ? 25 : 70 });
}

/** 通过 PowerShell 截取全屏（降级方案） */
async function takeSystemShot(opts = {}) {
  const tmp = path.join(writableTempDir(), `czj_sv_${Date.now()}.jpg`);
  const fastMode = !!opts.fastMode;
  const maxWidth = fastMode ? 320 : 0;
  const quality = fastMode ? 25 : 80;
...
$source.Save($ms,$codec,$params)
$bytes=$ms.ToArray()
[System.IO.File]::WriteAllBytes('${tmp.replace(/\\/g, '\\\\')}', $bytes)
if ($source -ne $b) { $source.Dispose() }
$g.Dispose();$b.Dispose();$ms.Dispose()
`.trim();
  // Base64-encode as UTF-16LE for -EncodedCommand (no file, no quoting issues)
  runEncodedPowerShell(script, 15000);
  const buf = fs.readFileSync(tmp);
  try { fs.unlinkSync(tmp); } catch {}
  return buf;
}

async function resizeJpeg(buffer, maxWidth = 320, quality = 25) {
  const input = path.join(writableTempDir(), `czj_sv_in_${Date.now()}_${Math.random().toString(16).slice(2)}.jpg`);
  const output = path.join(writableTempDir(), `czj_sv_out_${Date.now()}_${Math.random().toString(16).slice(2)}.jpg`);
  fs.writeFileSync(input, buffer);
  const script = `
$ProgressPreference='SilentlyContinue'
Add-Type -AssemblyName System.Drawing
$b=[System.Drawing.Bitmap]::FromFile('${psPath(input)}')
$newW=${maxWidth}
$newH=[int]($b.Height * ($newW / $b.Width))
$r=New-Object System.Drawing.Bitmap($newW,$newH)
$g=[System.Drawing.Graphics]::FromImage($r)
$g.InterpolationMode=[System.Drawing.Drawing2D.InterpolationMode]::Low
$g.DrawImage($b,0,0,$newW,$newH)
$codec=[System.Drawing.Imaging.ImageCodecInfo]::GetImageEncoders() | Where-Object { $_.MimeType -eq 'image/jpeg' } | Select-Object -First 1
$params=New-Object System.Drawing.Imaging.EncoderParameters(1)
$params.Param[0]=New-Object System.Drawing.Imaging.EncoderParameter([System.Drawing.Imaging.Encoder]::Quality,[long]${quality})
$r.Save('${psPath(output)}',$codec,$params)
$g.Dispose();$r.Dispose();$b.Dispose()
`.trim();
  try {
    runEncodedPowerShell(script, 15000);
...
async function windowsOcr(buffer) {
  const tmp = path.join(writableTempDir(), `czj_sv_ocr_${Date.now()}_${Math.random().toString(16).slice(2)}.jpg`);
...
function AwaitOp($op, $type) { $task=$asTaskGeneric.MakeGenericMethod($type).Invoke($null,@($op)); $task.Wait(); $task.Result }
$file=AwaitOp ([Windows.Storage.StorageFile]::GetFileFromPathAsync('${psPath(tmp)}')) ([Windows.Storage.StorageFile])
...
if ($null -eq $engine) { '' } else { $result=AwaitOp ($engine.RecognizeAsync($bitmap)) ([Windows.Media.Ocr.OcrResult]); $result.Text }
`.trim();
  try {
    const encoded = Buffer.from(script.trim(), 'utf16le').toString('base64');
    ...
    const raw = stdout || '';
    return raw.replace(/#< CLIXML[\s\S]*?<\/Objs>/g, '').trim();
  } catch (e) {
    console.warn('[眨眼] Windows OCR失败:', e.message);
    return '';
  } finally {
    try { fs.unlinkSync(tmp); } catch {}
  }
...
async function blinkOnce(task, opts = {}) {
  const { forceSystem = false, screenIndex = 1, prevSummaries = [], fastMode = false } = opts;
  ...
  try {
    buf = useBrowser ? await takeBrowserShot({ fastMode: false }) : await takeSystemShot({ fastMode: false });
  } catch (e) {
    if (useBrowser) { useBrowser = false; buf = await takeSystemShot({ fastMode: false }); }
    else throw e;
  }
  const captureMs = timingMs(captureStartedAt);

  const ocrStartedAt = Date.now();
  let ocrFinishedAt = 0;
  let ocrTimedOut = false;
  const ocrPromise = windowsOcr(buf).then((text) => {
    ocrFinishedAt = Date.now();
    return text;
  });
  const boundedOcrPromise = fastMode
    ? withTimeout(ocrPromise, 1500, '__OCR_TIMEOUT__').then((text) => {
      if (text === '__OCR_TIMEOUT__') {
        ocrTimedOut = true;
        return '';
      }
      return text;
    ...
    : ocrPromise;
  const visionInputPromise = fastMode ? resizeJpeg(buf, 320, 25) : Promise.resolve(buf);

  const prevCtx = !fastMode && prevSummaries.length
    ? `\n\n前面已分析的内容摘要：\n${prevSummaries.map((s, i) => `【第${i + 1}张】${s}`).join('\n')}\n\n请只提取本张截图中新出现的内容。`
    : '';

  const prompt = fastMode
    ? `companion-context fast glance.
这是第${screenIndex}张屏幕截图。用户任务：${task}${prevCtx}

只返回 1 句中文：屏幕主画面正在发生什么。
不要 OCR，不要列表，不要解释，不要写报告。`
    : `companion-context screen glance + full-screen OCR.
这是第${screenIndex}张屏幕截图。用户任务：${task}${prevCtx}
...
不要调用或建议调用别的工具。不要写分析报告，不要猜测截图外的信息。`;
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    try {
      const [visionInput, ocrText] = await Promise.all([visionInputPromise, boundedOcrPromise]);
      finalOcrText = ocrText;
      const enrichedPrompt = ocrText
        ? `${prompt}\n\n本地 OCR 文字：\n${ocrText}`
        : prompt;
      result = await callVision(visionInput, enrichedPrompt, fastMode ? 512 : 5000);
      break;
    } catch (e) {
      const is429 = e?.response?.status === 429 || /429/.test(e.message);
      if (is429 && attempt < 2) {
  ...
  const visionMs = timingMs(visionStartedAt);
  const ocrMs = ocrTimedOut ? 1500 : timingMs(ocrStartedAt, ocrFinishedAt || Date.now());

  console.log(`[眨眼] 第${screenIndex}张完成 (${result.length}字) capture=${captureMs}ms ocr=${ocrMs}ms vision=${visionMs}ms total=${timingMs(totalStartedAt)}ms fast=${fastMode}`);
  return { content: result, ocrText: finalOcrText, useBrowser, timings: { captureMs, ocrMs, ocrTimedOut, visionMs, totalMs: timingMs(totalStartedAt), fastMode } };
}

module.exports = { analyzeAndArchive, blinkOnce, takeBrowserShot, takeSystemShot, resizeJpeg, windowsOcr };
